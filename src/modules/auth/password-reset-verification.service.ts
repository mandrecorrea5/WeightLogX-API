...
export class PasswordResetVerificationService {
  private readonly logger = new Logger(PasswordResetVerificationService.name);
  ...
  constructor(
    @InjectRepository(PasswordResetVerificationEntity)
    private readonly verificationRepository: Repository<PasswordResetVerificationEntity>,
  ...
  ): Promise<PasswordResetVerificationEntity> {
  ...
  async verifyCode(
    ...
    if (!verification) {
      throw new NotFoundException(
        await safeTranslate(this.i18n, 'auth.forgotPassword.verificationNotFound', {
          lang: locale,
    ...
    if (verificationId && verification.id !== verificationId) {
      throw new BadRequestException(
        await safeTranslate(this.i18n, 'auth.forgotPassword.invalidVerificationId', {
          lang: locale,
    ...
    if (verification.method !== method) {
      throw new BadRequestException(
        await safeTranslate(this.i18n, 'auth.forgotPassword.invalidMethod', {
          lang: locale,
          defaultValue: 'Método de verificação inválido.',
        }),
      );
    }
    ...
    if (verification.expiresAt.getTime() < Date.now()) {
      throw new BadRequestException(
        await safeTranslate(this.i18n, 'auth.forgotPassword.codeExpired', {
          lang: locale,
    ...
    if (!isValid) {
      ...
      await this.verificationRepository.save(verification);
      throw new BadRequestException(
        await safeTranslate(this.i18n, 'auth.forgotPassword.codeInvalid', {
          lang: locale,
  ...
  async isVerified(userId: string): Promise<boolean> {
    ...
    // Verificar se foi verificado e não expirou
    if (!verification.verifiedAt) {
      return false;
    }

    // Verificar se a verificação ainda é válida (não expirou após verificação)
    const verificationValidWindow = 30 * 60 * 1000; // 30 minutos após verificação
    const timeSinceVerification = Date.now() - verification.verifiedAt.getTime();
    
    return timeSinceVerification < verificationValidWindow;
  }
  ...
  private async enforceResendLimit(
    verification: PasswordResetVerificationEntity,
    locale: string,
  ): Promise<void> {
    ...
    if (verification.resendCount >= this.resendLimitPerHour) {
      throw new HttpException(
        await safeTranslate(this.i18n, 'auth.forgotPassword.resendLimit', {
          lang: locale,
          defaultValue:
  ...
  private async resolveTarget(
    ...
    if (!sanitized) {
      throw new BadRequestException(
        await safeTranslate(this.i18n, 'auth.forgotPassword.phoneRequired', {
          lang: locale,
